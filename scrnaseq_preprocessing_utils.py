...
def get_proportions(adata, sample_obs, patient_obs, celltype_obs, comparison_obs):
    '''
    ...
    '''
    try:
        cell_type_df = adata.obs[[sample_obs, patient_obs, celltype_obs, comparison_obs]]
    except:
...
def plt_gene_heatmap_df(adata,pheno_heat,marker_genes,cluster_minus_one_present=False, zscore=True, layer_heat = False):
    ...
    if layer_heat == False:
        ...
        if zscore:
            gene_expression_zscored = pd.DataFrame(scipy.stats.zscore(np.array(adata.obs[marker_genes].astype(float)), axis=0),index=adata.obs_names,columns=marker_genes)
        else:
            gene_expression_zscored = pd.DataFrame(np.array(adata.obs[marker_genes].astype(float)), index=adata.obs_names, columns=marker_genes)
    ...
    marker_df = pd.DataFrame(columns=marker_genes)
